...
| --- | --- |
| `domain/models.py` | Contratos: parcela, huella, fachadas, vanos, regiones, salientes, escaleras, materiales y techo |
| `procedural_modeling/layout.py` | Retiro por frente y composición determinista de planta baja/pisos |
| `procedural_modeling/grammar.py` | Muros con vanos, regiones de material, marcos/salientes, escaleras, cubierta y jardín |
| `procedural_modeling/mesh_builder.py` | Sólidos recortados y triangulación restringida |
| `procedural_modeling/io.py` | Lectura de especificaciones JSON v1/v2/v3 |
...
- `parcel_xy` es el límite legal, `footprint_xy` la huella construida. No son
  intercambiables cuando existen retiros. Si se proporciona `footprint`, se usa
  exactamente; si no, `layout.py` propone una por retiros. `parcel - footprint`
  queda como superficie libre para acceso, jardín, cerco o escaleras exteriores.
  Ambos polígonos pueden tener patios/huecos.
- `appearance.materials` define slots PBR coherentes para todo el edificio. Cada
  fachada puede superponer `material_regions`, `projections` (incluido frente
  curvo) y `exterior_stairs`, siempre en coordenadas métricas locales `(u,v)`.
...
8. Superponer zonas cromáticas independientes de la estructura y ensamblar
   paneles, marcos, aleros, marquesinas curvas y escaleras con descansos/barandas.
9. Resolver materiales por slots semánticos (`plaster`, `accent`, `glass`,
   `metal`, etc.) con color, roughness, metallic, opacity y referencia de textura.
