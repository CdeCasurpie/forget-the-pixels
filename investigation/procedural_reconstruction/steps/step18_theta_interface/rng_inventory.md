...
| src/modeling/families.py:15 | apply_family | np.random.default_rng(spec.seed) | UNUSED | legacy_only | Seed/stream creation. Not reached by V4; preserved for old API |
...
| src/modeling/grammar.py:963 | generate_mesh | np.random.default_rng(spec.seed) | UNUSED | legacy_only | Seed/stream creation. Not reached by V4; preserved for old API |
| src/modeling/grammar.py:1182 | generate_v4_mesh | np.random.default_rng(spec.seed) | THETA | V4 | Seed/stream creation. Shared architectural stream; facade services and cladding |
...
| src/modeling/layout.py:87 | propose_building | np.random.default_rng(seed) | UNUSED | legacy_only | Seed/stream creation. Not reached by V4; preserved for old API |
...
| src/modeling/massing.py:241 | generate_masses | np.random.default_rng(program.seed if seed is None else seed) | THETA | V4 | Seed/stream creation. Mass pattern, split, setback or addition; never nuisance |
...
| src/modeling/materials.py:71 | appearance_for_style | np.random.default_rng(seed) | XI | V4 | Seed/stream creation. Foliage, rebar or small color variation; not massing |
...
| src/modeling/roofscape.py:214 | plan_roof | np.random.default_rng(resolve_seed(seed, lot_id, mass.id, "roof", "plan")) | THETA | V4 | Seed/stream creation. Roof class/depth/parapet or significant rooftop object layout |
| src/modeling/roofscape.py:214 | plan_roof | resolve_seed(seed, lot_id, mass.id, "roof", "plan") | THETA | V4 | Seed/stream creation. Roof class/depth/parapet or significant rooftop object layout |
...
| src/modeling/roofscape.py:260 | plan_roof | np.random.default_rng(                     resolve_seed(seed, lot_id, mass.id, "roof", "props")                 ) | THETA | V4 | Seed/stream creation. Roof class/depth/parapet or significant rooftop object layout |
| src/modeling/roofscape.py:395 | _build_roof_body | np.random.default_rng(prop.seed) | XI | V4 | Seed/stream creation. Object internal variation, object identity already planned |
...
| src/modeling/roofscape.py:173 | scatter_props | rng.uniform(0, step) | THETA | V4 | Roof class/depth/parapet or significant rooftop object layout |
| src/modeling/roofscape.py:261 | plan_roof | resolve_seed(seed, lot_id, mass.id, "roof", "props") | THETA | V4 | Seed/stream creation. Roof class/depth/parapet or significant rooftop object layout |
| src/modeling/roofscape.py:205 | scatter_props | rng.integers(0, 2**31 - 1) | THETA | V4 | Roof class/depth/parapet or significant rooftop object layout |
...
| src/modeling/theta.py:400 | _rng | np.random.default_rng(int.from_bytes(digest[:8],"little")) | XI | candidate | Seed/stream creation. Only curtain/foliage/rebar streams; roof object identity explicit |
| src/modeling/theta.py:279 | _composition | np.random.default_rng(0) | CONFIG | candidate | Seed/stream creation. Fixed RNG for family relief; sampled depths overwritten by theta |
| src/modeling/theta.py:432 | generate_resolved | _rng(nuisance.seed,f"roof/{roof.mass_id}/{prop.kind}/{prop.position}").integers(0,2**31) | XI | candidate | Only curtain/foliage/rebar streams; roof object identity explicit |
| src/modeling/theta.py:422 | generate_resolved | _rng(nuisance.seed,f"{f.edge_id}/curtain/{i}").uniform(.15,.55) | XI | candidate | Only curtain/foliage/rebar streams; roof object identity explicit |
