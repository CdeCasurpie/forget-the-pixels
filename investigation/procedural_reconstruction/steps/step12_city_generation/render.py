...
def render(mesh, path, direction=(1, -1.7, 1.1), size=900, clay=True, checker=False):
    ...
    if checker:
        # Reconstruct barycentric coordinates at visible surface samples.
        # Interpolate UVs per pixel, never assign one color per triangle.
        position = camera @ basis
        visible = tri[indices]
        e0, e1 = visible[:, 1]-visible[:, 0], visible[:, 2]-visible[:, 0]
        delta = position-visible[:, 0]
        dot = lambda a, b: np.einsum("ij,ij->i", a, b)
        aa, ab, bb = dot(e0,e0), dot(e0,e1), dot(e1,e1)
        da, db = dot(delta,e0), dot(delta,e1)
        det = aa*bb-ab*ab
        s, t = (bb*da-ab*db)/det, (aa*db-ab*da)/det
        uvtri = mesh.uv[mesh.faces[indices]]
        uv = uvtri[:,0] + s[:,None]*(uvtri[:,1]-uvtri[:,0]) + t[:,None]*(uvtri[:,2]-uvtri[:,0])
        scales = np.array([m.get("real_scale_m", 1) for m in mesh.materials])
        uv *= scales[mesh.face_materials[indices],None]
        tiles = np.floor(uv + 1e-6).sum(axis=1).astype(int) % 2
        rgb[yy,xx] = np.where(tiles[:,None] == 0, .85, .13) * illumination[:,None]
