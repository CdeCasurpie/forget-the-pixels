...
def analyze(
    ...
    for position, pano in enumerate(metadata):
        ...
        # La posición se conserva aunque caiga dentro de un lote: únicamente
        # se ignora ese lote durante el test de oclusión.
        camera_position_valid = True
    ...
    # Selección deliberadamente simple: las candidatas son las cámaras más
    # cercanas al polígono objetivo. La visibilidad queda reportada como
    # diagnóstico para inspección posterior.
    ranking = sorted(records, key=lambda record: (record["camera_to_lot_m"], record["distance_to_edge_m"]))
    selected = ranking[:top_k]
    top_k = len(selected)
    for rank, record in enumerate(ranking, start=1):
        record["camera_rank"] = rank
        record["selected_top_k"] = record in selected
    ...
    report = {
        ...
        "selection_criterion": "top_k cameras by camera_to_lot_m",
        "top_k": top_k,
        "top_camera_sequences": [record["sequence"] for record in selected],
        "panoramas": records,
    }
    ...
    selected_text = ", ".join(
        f"#{record['sequence']} ({record['camera_to_lot_m']:.1f} m, score={record['camera_score']})"
        for record in selected
    )
    print("Top 5 más cercanas: " + (selected_text or "ninguna cámara"))
...
def draw_map(gdf: gpd.GeoDataFrame, target_index, records: list[dict], path: Path) -> None:
    ...
    for record in records:
        ...
        alpha = 1.0 if record.get("selected_top_k") else 0.28
        width = 2.2 if record.get("selected_top_k") else 1.0
        ...
        ax.scatter(camera.x, camera.y, color="#111111", s=35 if record.get("selected_top_k") else 15, zorder=7)
        label = f"{record['sequence']} ({record['camera_to_lot_m']:.1f}m)" if record.get("selected_top_k") else str(record["sequence"])
        ax.text(camera.x + 2, camera.y + 2, label, fontsize=8, fontweight="bold" if record.get("selected_top_k") else "normal", alpha=alpha, zorder=8)
    ...
    ax.set_title("Paso 2 — Top 5 cámaras más cercanas al lote")
