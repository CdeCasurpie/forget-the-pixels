...
print("\033[96m[INFO]\033[0m Iniciando Semantic Mesher PROYECTADO...")
...
def key_mesh(vis):
    ...
    print("\033[95m[MALLA]\033[0m Ejecutando RANSAC y Proyección a Plano...")
    ...
    for seg_id in segs_to_process:
        ...
        try:
            # Tolerancia de 20cm para la pared
        ...
        # === NUEVO: PROYECCIÓN AL PLANO ===
        # plane_model es de la forma: ax + by + cz + d = 0
        A, B, C, D = plane_model
        normal = np.array([A, B, C])
        normal /= np.linalg.norm(normal)
        
        # d_vals es la distancia de cada punto al plano
        d_vals = (inl_pts3d @ normal) + D
        
        # Proyectamos restando la distancia a lo largo de la normal
        projected_pts3d = inl_pts3d - np.outer(d_vals, normal)
        # ==================================
        
        try: tri = Delaunay(inl_pts2d)
        except: continue
            
        all_verts.extend(projected_pts3d)
        all_cols.extend(inl_cols)
        all_tris.extend(tri.simplices + offset)
        offset += len(projected_pts3d)
        
    if len(all_verts) > 0:
        if inspect_mode:
            cache_inspector_mesh = (np.array(all_verts), np.array(all_tris), np.array(all_cols))
            print(f"\033[95m[MALLA]\033[0m Creada malla LOCAL (Proyectada) con {len(all_verts)} vertices.")
        else:
            cache_meshes[img_obj.image_id] = (np.array(all_verts), np.array(all_tris), np.array(all_cols))
            print(f"\033[95m[MALLA]\033[0m Creada malla TOTAL (Proyectada) con {len(all_verts)} vertices.")
    else:
...
print(" --- NAVEGACIÓN ---")
print("  - 'N' / 'B' : Cambiar Cámara (Siguiente / Anterior)")
...
print(" --- ALGORITMO ---")
print("  - 'S'       : Segmentar Color")
print("  - 'O' / 'P' : Inspector (Ver Segmento Siguiente / Anterior)")
print("  - 'U'       : Apagar Inspector")
print("  - 'R'       : Ransac + Proyección + Malla")

vis = o3d.visualization.VisualizerWithKeyCallback()
vis.create_window(window_name="Semantic Mesher (Projected)", width=1280, height=720)
